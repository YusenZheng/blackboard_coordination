...
def run() -> Harness:
    ...
    h.blackboard.append(BlackboardEvent(
        id=f"post-{task.task_id}", type=EventType.TASK_POSTED, ledger=Ledger.TASK,
        content={"task_id": task.task_id, "goal": task.goal}, source="ingress"))
    ...
    h.blackboard.append(BlackboardEvent(
        ...
        content={"task_id": task.task_id, "device_id": winner.device_id}, source="coord"))

    # 5. 中标者产出动作意图 → 安全 check(R0 放行)→【中标者的 loop 自己发起 tool_call】→ 回执回黑板
    intent = ActionIntent(intent_id="mv-1", device_id=winner.device_id,
                          verb=ActionVerb.MOVE_TO, params={"target": "waypoint_A"}, reversible=True)
    ...
    intent2 = ActionIntent(intent_id="enter-1", device_id="dog_a",
                           verb=ActionVerb.MOVE_TO,
                           params={"target": "绿篱后缝隙", "enclosed": True}, reversible=False)
    ...
    h.blackboard.append(BlackboardEvent(id="claim-a", type=EventType.CLAIM, ledger=Ledger.RECEIPT,
                                        content={"resource": "passage_X", "device_id": "dog_a"}, source="dog_a"))
    ...
    clue = Clue(clue_id="clue-1", position=Position(label="waypoint_B", area="片区B"),
                confidence=Confidence(level=ConfidenceLevel.MID, score=0.6, method="mock"),
                source_device=winner.device_id)
    h.blackboard.append(BlackboardEvent(
        id="clue-1", type=EventType.CLUE, ledger=Ledger.EVIDENCE,
        content={"clue_id": clue.clue_id, "position": clue.position.label,
    ...
    h.telemetry.push(TelemetrySample(device_id=winner.device_id, battery=0.8,
                                     position={"label": "waypoint_A"}, current_action="搜索中"))
    ...
    blocked = h.loops()[winner.device_id].act(
        ActionIntent(intent_id="after-estop", device_id=winner.device_id,
                     verb=ActionVerb.MOVE_TO, params={"target": "waypoint_C"}, reversible=True))
    print(f"  [急停门控] 急停中再发动作 → success={blocked.success}({blocked.failure_reason})")
    h.estop.clear("all")
    print(f"  [急停解除] 解除后动作可恢复下发")
    h.blackboard.append(BlackboardEvent(
        id=f"done-{task.task_id}", type=EventType.TASK_DONE, ledger=Ledger.TASK,
        content={"task_id": task.task_id}, source="coord"))
