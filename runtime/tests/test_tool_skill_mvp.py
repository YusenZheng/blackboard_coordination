...
from swarm_brain.assets.skill import load_builtin_skills
from swarm_brain.assets.emergence import CandidateLifecycle, CandidateState
from swarm_brain.assets.skill import Skill, SkillCandidate
    ...
    return AgentCard(
        ...
        capability=CapabilitySlot(
            ...
            profile=CapabilityProfile(
                capabilities=list(capabilities or ["search"]), width_cm=40
            ),
        ),
    )
...
class ToolSkillMvpTest(unittest.TestCase):
    def test_capability_match_is_explainable(self) -> None:
        result = CapabilityMatcher().match(
            make_card(capabilities=["search"]),
            CapabilityRequirement(all_of=frozenset({"sensor:thermal"})),
        )
    ...
    def test_g01_binds_agent_card_and_emits_trace(self) -> None:
        registry = Registry()
    ...
    def test_schema_alias_and_idempotency(self) -> None:
        ...
        invalid = gateway.invoke(
            ToolCallRequest("2.0", "bad", "S11", {}, idempotency_key="bad")
        )
        self.assertEqual("INVALID_ARGUMENT", invalid.error_code)

        first = gateway.invoke(
            ToolCallRequest(
                "2.0", "call-1", "S11", {"battery": 0.8}, idempotency_key="same"
            )
        )
        replay = gateway.invoke(
            ToolCallRequest(
                "2.0", "call-2", "S11", {"battery": 0.8}, idempotency_key="same"
            )
        )
        conflict = gateway.invoke(
            ToolCallRequest(
                "2.0", "call-3", "S11", {"battery": 0.1}, idempotency_key="same"
            )
        )
    ...
    def test_skill_registry_returns_validated_references(self) -> None:
        ...
        references = provider.search(
            SkillQuery(task_type="search_target", capability_ids=["search"])
        )
        self.assertEqual(2, len(references))
        self.assertTrue(all(reference.tool_chain for reference in references))
        self.assertTrue(
            all(set(reference.tool_chain) <= {"G01", "S11", "C03", "Y06"} for reference in references)
        )
