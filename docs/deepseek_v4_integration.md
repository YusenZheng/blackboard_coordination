...
- 输出模式：JSON object
- 密钥来源：进程环境或仓库根目录 `.env.local`

`.env.local` 已被 Git 忽略，不得把密钥写入源码、黑板事件、Session、Outbox
或日志。
...
脚本使用隐藏输入读取密钥并生成 `.env.local`。切换 Pro：

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\configure_deepseek.ps1 -Model deepseek-v4-pro
```
...
```json
{"status":"ok","model":"deepseek-v4-flash","base_url":"https://api.deepseek.com"}
```
...
页面下方的 LLM 调用明细来自 DeepSeek `/chat/completions` 响应中的 `usage`：
