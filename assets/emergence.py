"""V2 Skill candidate lifecycle contracts."""
...
class SimulationEvaluatorPort(Protocol):
    def evaluate(self, candidate: SkillCandidate, scenarios: list[dict]) -> ValidationResult: ...


class AdmissionGatePort(Protocol):
    def evaluate(self, candidate: SkillCandidate, validation: ValidationResult) -> AdmissionDecision: ...
    def rollback(self, skill_id: str, version: str, reason: str) -> None: ...


class CandidateLifecycle:
    _allowed = {
        CandidateState.CANDIDATE: {CandidateState.CANDIDATE_TRIAL, CandidateState.REJECTED},
        CandidateState.CANDIDATE_TRIAL: {CandidateState.VALIDATED, CandidateState.REJECTED},
        CandidateState.VALIDATED: {CandidateState.REGISTERED},
        CandidateState.REGISTERED: {CandidateState.CANARY, CandidateState.RETIRED},
        CandidateState.CANARY: {CandidateState.REGISTERED, CandidateState.ROLLED_BACK},
    }

    def transition(self, candidate: SkillCandidate, target: CandidateState) -> SkillCandidate:
